(function (global) {
  const ACCESS_MODULE_LABELS = {
    ...
    r_asset: "Carga de vehiculos",
    r_attach: "Adjuntos",
    r_auction_resolution: "Resultados",
    r_event: "Eventos de subasta",
    r_bid: "Historial de ofertas",
    r_module: "Modulos",
    r_company: "Companias",
    r_role: "Roles",
    r_ui_theme: "Tema UI",
  ...
  function moduleLabel(moduleKey) {
    return ACCESS_MODULE_LABELS[moduleKey] || moduleKey;
  }

  global.init_r_access = async function init_r_access() {
    const moduleById = new Map();
    const roleById = new Map();

    async function loadLookups() {
      const [modulesResponse, rolesResponse] = await Promise.all([
        global.SD_API.request("/api/r_module?includeInactive=true&take=200"),
        global.SD_API.request("/api/r_role?includeInactive=true&take=200"),
      ]);
    ...
    await global.SD_CRUD.mount({
      title: "Accesos",
      description: "Permisos CRUD por modulo y rol.",
      endpoint: "/api/r_access",
      listEndpoint: "/api/r_access?take=200",
      idField: "id_access",
      pageSize: 10,
      enableSearch: false,
      formHelp: "Usa esta pantalla para definir create, update y delete por modulo. Additional es opcional.",
      filters: [
        {
          name: "id_module",
          label: "Modulo",
      ...
      columns: [
        ...
        { key: "id_module", label: "Modulo", render: (row) => moduleDisplayById(row.id_module) },
