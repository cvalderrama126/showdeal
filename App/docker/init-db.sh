...
#
# Variables de entorno disponibles (desde docker-compose):
#   POSTGRES_USER     -> usuario de BD (default: showdeal)
#   POSTGRES_DB       -> nombre de BD (default: showdeal)
#   ADMIN_EMAIL       -> email del admin inicial (default: admin@showdeal.com)
#   ADMIN_FULL_NAME   -> nombre del admin (default: Admin User)
#   ADMIN_PHONE       -> teléfono del admin (default: +1-555-0000)
# ============================================

set -e

DB_USER="${POSTGRES_USER:-showdeal}"
DB_NAME="${POSTGRES_DB:-showdeal}"
...
-- Usuario Admin inicial (configurable via env vars)
...
-- ============================================
GRANT ALL PRIVILEGES ON SCHEMA showdeal TO ${DB_USER};
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA showdeal TO ${DB_USER};
GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA showdeal TO ${DB_USER};
GRANT USAGE ON SCHEMA showdeal TO ${DB_USER};

-- Asegurar que futuras tablas también tengan permisos
ALTER DEFAULT PRIVILEGES IN SCHEMA showdeal
...
echo "=== ShowDeal DB Init Complete ==="
echo "  Admin user created: $ADMIN_EMAIL"
echo "  Grants assigned to: $DB_USER"
echo "================================="
